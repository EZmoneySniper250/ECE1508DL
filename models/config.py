"""
Configuration for Student-CNN model.

Student-CNN uses a lightweight CNN backbone with dilated convolutions
for feature interaction, replacing the attention mechanism in LoFTR.
"""
...
class StudentCNNConfig:
    ...
    # ── Student backbone ─────────────────────────────────────────────────────
    backbone_channels = [32, 64, 128]  # stem → stage1 → stage2/stage3
    coarse_dim = 128           # 1/8 resolution coarse feature channels
    fine_dim = 64              # 1/2 resolution fine feature channels
